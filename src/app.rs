...
_window: Option<Arc<std::sync::Mutex<winit::window::Window>>>,
        ...
        let game_scene = GameSceneRenderer::new(&gl_context, paths.assets_dir.clone(), None)
        ...
        match &next {
            ...
            AppState::Playing => {
                if let Some(ref mut scene) = self.game_scene {
                    ...
                    if let Some(ref loaded) = self.loading.as_ref().and_then(|l| l.loaded.as_ref()) {
                        scene.set_world(&self.gl_context, loaded.world.clone());
                    }
                }
                self.lock_mouse();
            }
