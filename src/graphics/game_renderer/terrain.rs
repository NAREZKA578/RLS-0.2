...
impl TerrainMesh {
    ...
    fn is_water(x: f32, z: f32) -> bool {
        ...
        if x > ob_river_north && x < ob_river_south + ob_river_width {
    ...
    fn is_sand_beach(x: f32, z: f32) -> bool {
        let ob_river_south = -11000.0;
        let ob_river_north = -13000.0;

        if x > ob_river_north - 100.0 && x < ob_river_south + 100.0 + 800.0 {
            ...
            if z > river_edge_z_min && z < river_edge_z_max {
                if (x > beach_zone_south - 80.0 && x < beach_zone_south) ||
                   (x > beach_zone_north && x < beach_zone_north + 80.0) {
                    return true;
                }
            }
        }
    ...
    fn generate_novosibirsk(width: f32, depth: f32, cells_x: i32, cells_z: i32) -> Mesh {
        ...
        let mut terrain_types: Vec<i32> = Vec::with_capacity(((cells_x + 1) * (cells_z + 1)) as usize);

        for z in 0..=cells_z {
            for x in 0..=cells_x {
                let px = -hw + x as f32 * step_x;
                let pz = -hd + z as f32 * step_z;
                terrain_types.push(Self::get_road_type(px, pz));
            }
        }
        ...
        for z in 0..=cells_z {
            for x in 0..=cells_x {
                let px = -hw + x as f32 * step_x;
                let pz = -hd + z as f32 * step_z;
                let idx = (z * (cells_x + 1) + x) as usize;
                let road_type = terrain_types[idx];
