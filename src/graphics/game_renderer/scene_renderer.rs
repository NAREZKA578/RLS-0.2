...
use crate::vehicle::{CameraMode, VehicleController, VehicleState};
use crate::world::{Heightmap, World, WorldLoader};
use glam::{Mat4, Vec3};
use glow::HasContext;
...
impl GameSceneRenderer {
    pub fn new(gl: &glow::Context, assets_dir: PathBuf, world: Option<&World>) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            debug_program,
            scene_uniforms: SceneUniforms {
                view: unsafe { gl.get_uniform_location(scene_program, "u_view") },
                projection: unsafe { gl.get_uniform_location(scene_program, "u_projection") },
                model: unsafe { gl.get_uniform_location(scene_program, "u_model") },
                light_dir: unsafe { gl.get_uniform_location(scene_program, "u_light_dir") },
                view_pos: unsafe { gl.get_uniform_location(scene_program, "u_view_pos") },
            },
            terrain_vao,
            terrain_vbo,
            terrain_ibo,
            terrain_index_count,
            heightmap: {
                let ground = world.as_ref().and_then(|w| Some(w.ground_params()));
                let (cells_x, cells_z, _) = ground.unwrap_or((50, 300, 0.0));
                let world_w = cells_x as f32 * 10.0;
                let world_d = cells_z as f32 * 10.0;
                tracing::info!("Heightmap: {}x{}, world {}x{}", cells_x, cells_z, world_w, world_d);
                Heightmap::new(cells_x as i32, cells_z as i32, world_w, world_d)
            },
            vehicle_mesh,
            vehicle_vao,
            vehicle_vbo,
            ...
            player_pos: Vec3::new(100.0, 1.8, 200.0),
            is_in_vehicle: false,
    ...
    fn create_mesh_buffers(
        ...
        String,
    > {
        Self::create_mesh_buffers_with_color(gl, mesh, true)
    }
    ...
    fn create_terrain_buffers(
        ...
        String,
    > {
        Self::create_mesh_buffers_with_color(gl, mesh, false)
    }

    fn create_mesh_buffers_with_color(
        gl: &glow::Context,
        mesh: &Mesh,
        include_color: bool,
    ) -> Result<
    ...
    pub fn set_world(&mut self, gl: &glow::Context, world: World) {
        let (cells_x, cells_z, _) = world.ground_params();
        let world_w = cells_x as f32 * 10.0;
        let world_d = cells_z as f32 * 10.0;
        tracing::info!("Updating heightmap: {}x{}, world {}x{}", cells_x, cells_z, world_w, world_d);
        self.heightmap = Heightmap::new(cells_x as i32, cells_z as i32, world_w, world_d);
        self.world = Some(world);
        let objects = Self::load_world_objects(gl, self.world.as_ref().unwrap(), &self.assets_dir);
        self.world_objects = objects;
    }

    pub fn set_character_height(&mut self, height_m: f32) {
        self.camera.position.y = height_m - 0.08;
        self.character_height = height_m;
    }
    ...
    pub fn render(&mut self, gl: &glow::Context) {
        ...
        unsafe {
            ...
            if let Some(loc) = &self.scene_uniforms.light_dir {
            ...
            if let Some(loc) = &self.scene_uniforms.view_pos {
            ...
            let draw_vehicle = !self.is_in_vehicle
                || matches!(self.camera_mode, CameraMode::ThirdPerson { .. });
            if draw_vehicle && self.vehicle_index_count > 0 {
                let model = VehicleController::model_matrix(&self.vehicle);
                if let Some(loc) = &self.scene_uniforms.model {
                    gl.uniform_matrix_4_f32_slice(Some(loc), false, &model.to_cols_array());
                }
                gl.bind_vertex_array(Some(self.vehicle_vao));
                gl.draw_elements(glow::TRIANGLES, self.vehicle_index_count as i32, glow::UNSIGNED_INT, 0);
            }
    ...
    pub fn update(&mut self, dt: f32, input: &crate::platform::input::InputState) {
        if self.is_in_vehicle {
            VehicleController::update(&mut self.vehicle, input, dt, &self.heightmap);
            crate::vehicle::apply_camera(&mut self.camera, &self.camera_mode, &self.vehicle, self.player_pos);
        } else {
            self.update_walking(dt, input);
        }
    }

    fn update_walking(&mut self, dt: f32, input: &crate::platform::input::InputState) {
        use winit::keyboard::KeyCode;

        let speed = 4.5_f32;

        let forward = input.is_key_down(KeyCode::KeyW) || input.is_key_down(KeyCode::ArrowUp);
        let back = input.is_key_down(KeyCode::KeyS) || input.is_key_down(KeyCode::ArrowDown);
        let left = input.is_key_down(KeyCode::KeyA) || input.is_key_down(KeyCode::ArrowLeft);
        let right = input.is_key_down(KeyCode::KeyD) || input.is_key_down(KeyCode::ArrowRight);

        let cam_forward = self.camera.forward();
        let cam_right = self.camera.right();

        let mut move_dir = Vec3::ZERO;
        if forward { move_dir += cam_forward; }
        if back { move_dir -= cam_forward; }
        ...
        let ground_y = self.heightmap.sample(self.player_pos.x, self.player_pos.z);
        self.player_pos.y = ground_y + self.character_height - 0.08;
        self.camera.position = self.player_pos;
    }

    pub fn toggle_vehicle_mode(&mut self) {
        if self.is_in_vehicle {
            let door_offset = self.vehicle.rotation * Vec3::new(1.5, 0.0, 0.3);
            self.player_pos = self.vehicle.position + door_offset;
            self.player_pos.y = self.heightmap.sample(self.player_pos.x, self.player_pos.z)
                + self.character_height;
            self.camera_mode = CameraMode::Walking;
            self.is_in_vehicle = false;
        } else {
            let dist = (self.player_pos - self.vehicle.position).length();
            if dist < 5.0 {
                self.camera_mode = CameraMode::cabin();
                self.is_in_vehicle = true;
            }
        }
    }

    pub fn get_terrain_y(&self, x: f32, z: f32) -> f32 {
