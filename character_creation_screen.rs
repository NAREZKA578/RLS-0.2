...
const UNIVERSITIES: [&str; 6] = ["НГТУ", "НГУ", "НГМУ", "НГАСУ", "НГПУ", "СибГУТИ"];
const SPECIALTIES: [&str; 4] = ["Инженерия", "Механика", "Экономика", "Право"];

const CAPITAL_TABLE: [[f64; 4]; 6] = [
    [40000.0, 55000.0, 70000.0, 90000.0],  // НГТУ
    [35000.0, 50000.0, 65000.0, 80000.0],  // НГУ
    [45000.0, 60000.0, 80000.0, 100000.0], // НГМУ
    [30000.0, 45000.0, 60000.0, 75000.0],  // НГАСУ
    [32000.0, 48000.0, 62000.0, 82000.0],  // НГПУ
    [38000.0, 52000.0, 68000.0, 88000.0],  // СибГУТИ
];
...
impl CharacterCreationScreen {
    ...
    fn selected_idx(&self) -> usize {
        ...
        match s {
            ...
            5 => HAIR_COLORS
                .iter()
                .position(|c| *c == self.data.hair_color)
                .unwrap_or(0),
            6 => UNIVERSITIES
                .iter()
            ...
            8 => REGIONS
                .iter()
                .position(|r| r == &self.data.start_region)
                .unwrap_or(0),
            _ => 0,
        }
    }
        ...
        if mouse_just_pressed {
            ...
            match s {
                ...
                5 => {
                    ...
                    let sx = a.x + (a.w - total) / 2.0;
                    let cy = a.y + a.h / 2.0 - bw / 2.0 - 16.0;
                    for (i, &color) in HAIR_COLORS.iter().enumerate().take(5) {
                        let r = Rect {
                            x: sx + i as f32 * (bw + gap),
                            y: cy,
                            w: bw,
    ...
    fn render_region(&self, batch: &mut DrawBatch, ui: &crate::graphics::UiRenderer, a: Rect) {
        ...
        let dot_colors = [
            [0.85, 0.55, 0.15],  // Центр - оранжевый
            [0.2, 0.6, 0.85],     // Ленинский - синий
            [0.3, 0.7, 0.4],     // Первомайский - зелёный
            [0.85, 0.3, 0.3],     // Кировский - красный
            [0.6, 0.3, 0.8],      // Советский - фиолетовый
            [0.85, 0.65, 0.2],    // Дзержинский - жёлтый
            [0.2, 0.5, 0.7],     // Залив - тёмно-синий
            [0.5, 0.5, 0.5],      // Нарымский - серый
            [0.7, 0.4, 0.6],      // Маршала Пожарова - розовый
            [0.4, 0.65, 0.3],     // Старый Сибирь - тёмно-зелёный
            [0.3, 0.5, 0.8],      // Октябрьский - голубой
            [0.8, 0.5, 0.3],      // Калининский - коричневый
        ];
